    ...
    public partial class FormEtudiant : Form
    {
        ...
        private void btnetudiantajouter_Click(object sender, EventArgs e)
        {
                ...
                && txtetudiantemail.Text.Length > 0)
            {
                ...
                using (var db = new DB())
                {
                    Classe classe = db.Classes.FirstOrDefault(c => c.Id == (int)txtetudiantclasse.SelectedValue);
                    etudiant.IdClasse = (int)txtetudiantclasse.SelectedValue;
...
private void txtetudiantsexe_SelectedIndexChanged(object sender, EventArgs e)
        ...
        private void btnetudiantmodifier_Click(object sender, EventArgs e)
        {
            using (var db = new DB())
            {
                ...
                && txtetudiantemail.Text.Length > 0)
                    {
                        if(etudiant.IdClasse != (int)txtetudiantclasse.SelectedValue)
                        {
                            supprimenote(etudiantId);
                        }
                        int classeId = ((Classe)txtetudiantclasse.SelectedItem).Id;
                        etudiant.Classe = db.Classes.FirstOrDefault(c => c.Id == classeId);
                        etudiant.IdClasse = classeId;
                        etudiant.Nom = txtetudiantnom.Text;
                        etudiant.Prenom = txtetudiantprenom.Text;
                        etudiant.Adresse = txtetudiantadresse.Text;
                        etudiant.Sexe = txtetudiantsexe.Text;
        ...
        private void supprimenote(int ide)
        {
            using (var db = new DB())
            {
                var notes = db.Notes.Where(n => n.IdEtudiant == ide).ToList();
                foreach (var note in notes)
                {
                    db.Notes.Remove(note);
                }
